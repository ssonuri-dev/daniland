...
- **`items` 모양에 안 맞는 콘텐츠는 `LESSONS` 를 늘리지 마세요.** `items` 의 단위는
  ...
  **`box` 끼리 겹치면 위에 그려진 단추가 아래 것을 가립니다.**
  ...
  ⚠️ **세계 지도는 폰에서 나라가 너무 작습니다.** 지리적으로 정확한 지도에서 한국은 가로의 4.5%
  라서, 폰(390px)에서는 16×18px 이 됩니다 (손가락 최소 64px). 태블릿 가로에서도 39×43px 입니다.
  좌표를 넓히는 것으로는 못 고칩니다 — 확대나 대륙→나라 2단계 같은 화면 설계가 필요합니다.
