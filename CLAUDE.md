...
index.html            과목 카드            home.js
 └ subject.html?name=영어   묶음 카드        subject.js  (SUBJECTS[].groups 가 있는 과목만 — 없으면 바로 아래 층)
   └ subject.html?name=영어&group=단어 공부   수업·놀이 카드   subject.js
      ...
      ├ country.html?country=sweden         country.js  (나라 한 곳 깊이 — 이야기 카드 18장·퀴즈. 데이터는 js/countries/<id>.js: 스웨덴·그리스·독일·프랑스. sweden.html 은 옛 주소 → 넘김)
...
| `daniland.best.greece.<act>` `daniland.best.greece` `daniland.best.germany.<act>` `daniland.best.germany` `daniland.best.france.<act>` `daniland.best.france` | 그리스·독일·프랑스 — 스웨덴과 같은 규칙 (나라 페이지는 `daniland.best.<id>`) |
