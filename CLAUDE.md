...
`lesson.wordKo` 를 켜면 **'글자 찾기'·'짝 맞추기' 에서** 영어 대신 우리말(`ko`)을 보여 주고
우리말로 읽습니다 (`game.js` 의 `koCards()`). 국기 수업(193개국)이 이것을 씁니다 — 국기를 보고
우리말 나라 이름을 고르는 놀이입니다. **'듣고 찾기' 만 `lesson.lang`(영어)을 그대로 씁니다**
— 영어를 듣고 국기를 찾는 연습은 남겨 두려는 것이니 여기까지 우리말로 바꾸지 마세요.
