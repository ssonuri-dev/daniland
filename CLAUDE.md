...
- **`items` 모양에 안 맞는 콘텐츠는 `LESSONS` 를 늘리지 마세요.** `items` 의 단위는
  ...
  **길이 동그라미가 아닌 것도 있습니다 (2026-09-23 더함).** 데이터에 `e`(길쭉한 정도)·`inc`(기울기)·`retro` 가 적힌 몸은
  케플러 방정식을 뉴턴법으로 풀어(`eccAnomaly`) 타원으로 돕니다 — **태양에 가까울 때 빨라지고 멀 때 느려지는 것이 그대로 보입니다.**
  명왕성(왜소행성, 17도 기울어진 길)과 핼리 혜성(거꾸로 돌고 꼬리는 늘 태양 반대쪽)이 그것이고, 화성·목성 사이에는 소행성대가
  점으로 흐릅니다 (누를 수 없고 이름표만 — 돌이 너무 작습니다). **'📏 진짜 거리' 칩**은 궤도를 실제 비율(`REAL_UNIT`)로 늘려
  '평소 화면은 진짜가 아니다' 를 눈으로 보여 줍니다 — 그때는 행성이 전부 점이 되므로 **구경하기에서만** 켤 수 있습니다.
  이름표는 가까운 것부터 적고 겹치면 건너뜁니다(`placeLabel`) — 진짜 거리 모드에서 안쪽 넷이 뭉쳐도 글자가 안 뭉개집니다.
...
`lines`(십자 대신 영어 공책 가로줄 0.14·0.46·0.78)가 붙습니다. 소문자는 `a` 로 읽히면 관사처럼
'어' 가 되는 목소리가 있어 `speak` 를 대문자로 둡니다. i·j 의 점은 `isDot()` 이 톡 찍기도 획으로 받습니다.

**화면 하나가 카드 셋을 섬깁니다** (`CARDS`) — 한글 '글자 쓰기'(`write.html`) · 영어 '알파벳 쓰기'(`?lang=en`) ·
영어 '단어 쓰기'(`?lang=en&kind=word`). 묶음마다 `card` 를 적어 두고, **시작 화면에는 들어온 카드의 묶음만 보입니다**
— 아이가 '알파벳 쓰기' 를 눌렀는데 한글이 나오면 헷갈립니다. ← 와 ⭐ 기록도 카드마다 갈립니다
(`daniland.best.write` · `.en` · `.enw`). `PAGES` 에서 세 카드는 **한글 → 알파벳 → 단어 쓰기** 차례여야 합니다 —
`Catalog.backHref()` 가 href 앞머리로 앞에서부터 찾기 때문입니다 (`write.html?lang=en` 은 `…&kind=word` 의 앞머리이기도 합니다).

**영어 낱말(`enw3`/`enw4`)은 소문자 획을 이어 붙입니다** (2026-09-24). 낱말을 여기 적지 않고 **영어 수업(`LESSONS`)에서
그대로 읽어 옵니다**(`enWords()`) — 글자 만들기(`make.js`)가 한글 수업을 읽는 것과 같은 규칙이라, 수업에 낱말을 더하면
쓸 낱말도 같이 늡니다. **여기에 낱말 배열을 새로 만들지 마세요.** 국기 수업은 나라 이름이라 뺍니다(`wordKo`).
글자 수가 곧 칸 수라 **세 글자·네 글자만** 씁니다 — 다섯 글자는 폰에서 칸이 손가락보다 작아집니다.
`fitCells()` 가 한 줄에 다 놓으면 `CELL_MIN`(120px) 아래로 떨어질 때 **줄을 나눕니다** (폰 세로에서 네 글자면 2 + 2 로
174px, 태블릿은 한 줄 189px). 아이는 태블릿을 주로 쓰므로 낱말은 대개 한 줄로 보입니다.
...
| `daniland.best.write.<set>` | 글자 쓰기 묶음별(자음·모음·숫자·낱말·대문자·소문자·세 글자·네 글자) 최고 별 |
...
| `daniland.best.write.enw` | 영어 낱말 묶음(세 글자·네 글자) 중 제일 잘한 기록 (영어의 '단어 쓰기' 카드가 읽습니다) |
...
- **부수기는 한 번 치면 터집니다 (2026-09-27 정함).** 처음엔 두세 번씩 쳐야 깨지고 세게 쳐야 한 칸이
  깎이게 했는데 '빡세다' 는 반응이라, 체력을 없애고 **개수를 늘렸습니다**(1단계 16마리, 판에 늘 너덧 마리).
  재미는 **터지는 순간의 효과**에서 나오게 했습니다 — 몸이 찌부러지고 젤리 방울·'뽁!'·고리·판 흔들림·선물,
  이어 터뜨리면 'n연속!' 과 한 음씩 오르는 소리(`SFX.smash(n)`). 체력을 되살리지 마세요.
  `power`(움직임의 세기)는 이제 **방울이 튀는 거리에만** 씁니다. 손 모드에서는 커서가 지나간 자리의
  괴물을 **모두** 터뜨립니다 — 쓸어 터뜨려 연속을 만드는 것이 재미라 하나만 고르지 않습니다.
- **괴물은 그림 파일이 아니라 css 로 그립니다** (`.slime` 안에 반짝이·눈·입·머리 장식, 크기는 전부 %).
  이모지로는 터질 때 몸이 찌부러지는 모습을 못 보여 줘서입니다. 사용자는 '귀여운 괴물이나 뿌요뿌요 덩어리' 를
  원했습니다 — 무서운 괴물로 바꾸지 마세요. 괴물을 더하려면 `smash.js` 의 `MONSTERS` 에 한 줄.
